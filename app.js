(function () {
    ...
    function weather() {
        ...
        function success(position) {
            ...
            $.getJSON(
                proxy + url + apiKey + "/" + latitude + "," + longitude + "?units=si",
                function (forecast) {
                    currenticon = forecast.currently.icon;
                    ctemp = Math.round(forecast.currently.temperature);
                    cdt = (function (currenticon) {
                        if (
                            forecast.currently.icon === "partly-cloudy-day" ||
                            "partly-cloudy-night"
                        )
                            return "cloudy";
                        else return currenticon;
                    })();
                    init();
                    //hide loading
                    $("#loading").fadeOut(200);
    ...
    function phrasemaker(ph) {
        ...
        processed.title += "\n<span id=subtext>\n" + ph.subline + "</span>";
        processed.effcolor = "linear-gradient(45deg, #dadadada 20%, #" + ph.color + " 40%, #" + ph.color + " 60%, #dadadada 80%)";
        return processed;
    }
    ...
    function showTemp(){
        $("#some").fadeOut();
        $("#some").html("It's\nFucking\n<span id=with-anim>" + ctemp + "º!</span>");
        $("#with-anim").css({ "background-image": cphrase.effcolor });
        $("#some").fadeIn();
